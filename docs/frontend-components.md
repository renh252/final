...
本文檔描述了寵物領養平台前端的元件結構。前端使用 Next.js 框架開發，採用 App Router 結構，並使用 React Bootstrap 和 Tailwind CSS(待修正:目前已棄用，因為會和 bootstrap 衝突) 作為 UI 框架。系統包含多個共用元件和頁面元件，以實現各種功能和頁面。
