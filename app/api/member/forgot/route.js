    ...
    switch (action) {
      ...
      case 'verify-otp':
        if (!email || !otp || !/^\d{6}$/.test(otp)) {
          return NextResponse.json(
            { message: '請提供電子郵件和有效的 6 位數驗證碼' },
            { status: 400 }
          )
        }
        try {
          const [tokens] = await database.execute(
            'SELECT token, expires_at FROM password_reset_tokens WHERE user_email = ?',
            [email]
          )
      ...
      case 'reset-password':
        if (!email || !otp || !newPassword || newPassword.length < 6) {
          return NextResponse.json(
            { message: '請提供電子郵件、驗證碼和長度至少為 6 個字元的新密碼' },
            { status: 400 }
          )
        }
        try {
          ...
          if (result && result.affectedRows > 0) {
            ...
            try {
              const [userRows] = await database.execute(
                'SELECT user_id, user_name FROM users WHERE user_email = ?',
                [email]
              )

              if (userRows && userRows.length > 0) {
                const userId = userRows[0].user_id
                const userName = userRows[0].user_name || '會員'
                ...
                await fetch(
                  ...
                  }/api/notifications/add`,
                  {
                    ...
                    body: JSON.stringify({
                      ...
                      type: 'security',
                      title: '密碼已成功重設',
                      message: `親愛的 ${userName}，您的帳戶密碼已成功重設。如果這不是您本人操作，請立即聯繫客服。`,
                      link: '/member',
                    }),
                  }
                )
              ...
              console.error('發送密碼重設通知時發生錯誤:', notifyError)
              // 不影響主流程
            }
