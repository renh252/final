...
export const config = {
    api: {
        bodyParser: false, // 禁用 Next.js 預設的 body parser，讓 formidable 處理
    },
};
...
async function saveFile(file, newFilename) {
    ...
    const newPath = path.join(process.cwd(), '/public/uploads/profile_photos', newFilename);
...
export async function POST(request) {
    try {
        ...
        if (decoded.userId) {
            ...
            const [userRows] = await database.execute('SELECT user_id FROM users WHERE firebase_uid = ?', [decoded.uid]);
            if (userRows.length === 0) {
                return NextResponse.json({ message: '找不到對應的 Google 登入使用者' }, { status: 404 });
            }
        ...
        if (Array.isArray(profilePhotoFile)) {
            return NextResponse.json({ message: '僅允許上傳單個圖片檔案' }, { status: 400 });
        }
        ...
        if (updateResult.affectedRows > 0) {
            // 查詢更新後的的使用者資料並返回
            const [userRows] = await database.execute('SELECT * FROM users WHERE user_id = ?', [userId]);
            ...
            return NextResponse.json({ message: '更新使用者資料失敗' }, { status: 500 });
        }
