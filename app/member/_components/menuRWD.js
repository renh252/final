...
export default function MenuRWD({children}) {
  const [menuOpen, setMenuOpen] = useState(false)
    ...
    const isCollectionActive = useCallback(() => {
      return ['/member/FavoritesList/Pets', '/member/FavoritesList/products', '/member/FavoritesList/articles'].some(path => pathname.startsWith(path))
    }, [pathname])
  ...
  return ReactDOM.createPortal(
    <>
      <div className={`${styles.menu} ${menuOpen ? styles.open : ''}`}>
        ...
        <button className={isActive('/member/appointments') ? styles.active : ''}>
          <Link href="/member/appointments">寵物預約</Link>
        </button>
        <div className={`${styles.collapsible} ${isCollectionActive() ? styles.active : ''}`}>
          <button onClick={() => setCollectionOpen(!collectionOpen)}>
            我的收藏
            {collectionOpen ? <MdExpandLess /> : <MdExpandMore />}
          </button>
          <div className={`${styles.collapsibleContent} ${collectionOpen ? styles.open : ''}`}>
            <button 
              className={isActive('/member/FavoritesList/Pets') ? styles.active : ''}
              onClick={() => handleCollectionClick('/member/FavoritesList/Pets')}
            >
              寵物
            </button>
            <button 
              className={isActive('/member/FavoritesList/products') ? styles.active : ''}
              onClick={() => handleCollectionClick('/member/FavoritesList/products')}
            >
              商品
            </button>
            <button 
              className={isActive('/member/FavoritesList/articles') ? styles.active : ''}
              onClick={() => handleCollectionClick('/member/FavoritesList/articles')}
            >
              文章
            </button>
          </div>
        ...
        <button className={isActive('/member/donations') ? styles.active : ''}>
          <Link href={user ? `/member/donations?=${user_id}` : '/member/donations'}>
            我的捐款
          </Link>
        </button>
