...
console.log(areaData['臺中市']);
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'mobile_barcode') {
      newValue = value === '' ? '/' : value.startsWith('/') ? value : '/' + value;
...
// }, [checkoutData]);
  ...
  const validateForm = () => {
    ...
    }else if(!checkoutData.recipient_phone.startsWith('0') || checkoutData.recipient_phone.length > 10){
    ...
    if(checkoutData.delivery === '宅配到府'){      
      // 地址
      if (!checkoutData.address.city || !checkoutData.address.town || !checkoutData.address.else) {
